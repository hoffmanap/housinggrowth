...
**Front end:** A single self-contained HTML file — no build step, no server. Chart.js and Leaflet are both embedded directly in the file rather than loaded from a CDN. The hero map is a real interactive Leaflet map — pannable, zoomable, with a clean grayscale CARTO basemap showing streets and place names — plotting every growth-area parcel by its actual latitude/longitude, with period and zip filters and hover tooltips. All data is embedded as JSON in the page. The map basemap tiles are the one piece that requires an internet connection to display (any web map does); everything else works fully offline once downloaded.
